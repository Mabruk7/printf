...
/**
 * write_num - Writs number using a bufffer
	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == ' ')/* extra character to left of the  buff */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
...
int write_unsgnd(int is_negative, int ind,
	...
	int flags, int width, int precision, int size)
{
	/* The num is stored at the bufer's right and starts at position i */
	...
	if (precision == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0')
		return (0); /* printf(".0d", 0)  no character are printed */
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		for (i = 3; i < width - length + 3; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ')/* Asign extra character to left of the buffer */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
